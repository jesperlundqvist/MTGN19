...
if (isset($_FILES['file'], $_POST['photographer'], $_POST['week'],$_POST['event'])) {
  ...
  if ($stmt = $link->prepare("INSERT INTO images (imagename, uploaddate, photographer, imagecreateddate, week, event) VALUES (?, ?, ?, ?, ?, ?)")) {
    $stmt->bind_param('ssssss', $imagename, $date, $photographer, $imagecreateddate, $week, $event);
    $stmt->execute();
    if ($stmt->error) {
